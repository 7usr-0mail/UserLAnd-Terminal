...
class FilesystemManager(
    ...
    /**
     * Copies the bootstrap scripts shipped in the APK into a filesystem's support
     * directory. These convert a pristine tarball from an official OS archive
     * into something that can actually run under proot.
     */
    fun stageBootstrapScripts(filesystem: Filesystem) {
        val ctx = context ?: return
        val supportDir = File("$filesDirPath/${filesystem.id}/support")
        supportDir.mkdirs()
        val scripts = listOf("bootstrapOfficialRootfs.sh", "installServiceManager.sh")
        for (script in scripts) {
            try {
                val target = File(supportDir, script)
                ctx.assets.open("bootstrap/$script").use { input ->
                    target.outputStream().use { output -> input.copyTo(output) }
                }
                ulaFiles.makePermissionsUsable(supportDir.absolutePath, script)
            } catch (err: Exception) {
                logger.addExceptionBreadcrumb(Exception("Could not stage $script"))
            }
        }
    }
    ...
    suspend fun extractFilesystem(
        ...
        // Consumed by the bootstrap script when the rootfs came from an official
        // archive and still needs to be adapted for proot.
        env["DISTRIBUTION_TYPE"] = filesystem.distributionType
        env["PACKAGE_MIRROR"] = packageMirror
