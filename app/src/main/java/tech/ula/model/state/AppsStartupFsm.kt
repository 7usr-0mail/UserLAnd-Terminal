...
class AppsStartupFsm(
    ...
    // Seed synchronously so an early event cannot observe a null state.
    private val state = MutableLiveData<AppsStartupState>().apply { value = WaitingForAppSelection }

    private fun currentState(): AppsStartupState = state.value ?: WaitingForAppSelection
    ...
    fun submitEvent(event: AppsStartupEvent, coroutineScope: CoroutineScope) = coroutineScope.launch {
        ...
        if (!transitionIsAcceptable(event)) {
            state.postValue(IncorrectAppTransition(event, currentState()))
            return@launch
        }
